...
public class OneScenarioResultPresenter extends Presenter<OneScenarioResultViewInterface>{
	...
	private static final Logger logger = Logger.getLogger("OneScenarioResultPresenter.class");
	...
	public void onCalculateOneScenario(OneScenarioCalculationEvent event) {
			...
			if (periodContainer instanceof CashFlowPeriodContainer) {
				...
				CashFlowCalculator.calculateUKVCashflows((UmsatzkostenVerfahrenCashflowPeriodContainer) periodContainer,
						scenario);
			} else if (periodContainer instanceof GesamtkostenVerfahrenCashflowPeriodContainer) {
				CashFlowCalculator.calculateGKVCashflows((GesamtkostenVerfahrenCashflowPeriodContainer) periodContainer,
						scenario);
			}
			...
			if (method.getName().equals("Flow-to-Equity (FTE)")) {
				...
				cc.add(String.valueOf(periods.last().getYear()), new double[] {
						Double.parseDouble(nfUS.format(unternehmenswert).replace(",", "")), dFremdkapital });
				...
				cc.add(String.valueOf(periods.last().getYear()), new double[] {
						Double.parseDouble(nfUS.format(unternehmenswert).replace(",", "")), dFremdkapital, 0, 0 });
				cc.add(String.valueOf(periods.last().getYear()),
						new double[] { 0, 0, Double.parseDouble(nfUS.format(uwSteuerfrei).replace(",", "")),
								Double.parseDouble(nfUS.format(steuervorteile).replace(",", "")) });
			...
			getView().setScenarioValue(nfDE.format(scenario.getRateReturnEquity()),
					nfDE.format(scenario.getRateReturnCapitalStock()), nfDE.format(scenario.getBusinessTax()),
					nfDE.format(scenario.getCorporateAndSolitaryTax()), nfDE.format(scenario.getPersonalTaxRate()));
