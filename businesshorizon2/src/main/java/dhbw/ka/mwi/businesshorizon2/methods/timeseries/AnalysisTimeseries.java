...
public class AnalysisTimeseries {
	 ...
	 */
	public double[][] prognoseBerechnenNew(DoubleArrayList trendbereinigtezeitreihe, double[] valuesForPhi, double standardabweichung, int zuberechnendeperioden, int iterationen, int p, double mittelwert, boolean isfremdkapital, double konstanteC) {
		...
		for (int h = 0; h < iterationen; h++) {// Durchführen der Durchläufe
			...
			for (int m = 0; m < zuberechnendeperioden; m++) {// Anzahl der
				...
				value = value + konstanteC;// ToDo konstanteC einen Wert
											// zuweisen
	...
	public double[][] calculate(double[] zeitreihe, int p, int zuberechnendePerioden, int durchlaeufe, CallbackInterface callback, boolean isfremdkapital) throws InterruptedException, StochasticMethodException {
		...
		this.standardabweichung = berechneStandardabweichung(autokovarianzen, modellparameter);
